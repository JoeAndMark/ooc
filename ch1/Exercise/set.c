...
/**
 * @brief apply action to every object in the _set.
 *
 * @param action [ in] : Callback function.
 * @param ap     [ in] : arguments list.
 * 
 * @return true if all elements were processed, and vice verse.
 *
 * @note action mustn't change the _set but it may return zero to termiate apply() early.
 */
bool apply(const void* _set, int(* action)(void* object, va_list ap), ...) {
    struct Set* set = (struct Set* )_set;
